...
extension FitTextStyleExtension on BuildContext {
  ...
  /// 폰트 크기 계산 (타입별 처리)
  /// ScreenUtil 없이 MediaQuery를 사용한 반응형 크기 계산
  double _getFontSize(double baseSize, FitTextSp type) {
    switch (type) {
      case FitTextSp.MIN:
        ...
        return baseSize;
      case FitTextSp.MAX:
        // MAX: MediaQuery 기반 최대 크기
        final screenWidth = MediaQuery.of(this).size.width;
        final scale = screenWidth / 390; // 디자인 기준 너비 390
        return baseSize * scale.clamp(1.0, 1.5); // 최대 1.5배까지만 확대
      case FitTextSp.SP:
        // SP: MediaQuery 기반 화면 비율 적용
        final screenWidth = MediaQuery.of(this).size.width;
        final scale = screenWidth / 390; // 디자인 기준 너비 390
